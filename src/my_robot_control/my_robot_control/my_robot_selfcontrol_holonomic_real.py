...
class RobotSelfControlHolonomic(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('distance_limit', 0.1)
        ...
        self._msg.linear.y = 0.0  # For holonomic: sideways motion
    ...
    def laser_callback(self, scan):
        ...
        # Holonomic reactions to obstacles
        if closest_distance < self._distanceLimit:
            if zone == "FRONT":
                ...
                # Strafe RIGHT (positive y)
                self._msg.linear.x = 0.1
                self._msg.linear.y = -self._strafeSpeed * self._speedFactor *0.5
                self._msg.angular.z = 0.0
            elif zone == "RIGHT":
                # Strafe LEFT (negative y)
                self._msg.linear.x = 0.1
                self._msg.linear.y = self._strafeSpeed * self._speedFactor *0.5
                self._msg.angular.z = 0.0
            elif zone == "BACK_LEFT":
